import {
    ...
    w as MeshStandardMaterial,
    C as Color,
    x as DoubleSide,
    I as InstancedMesh,
    ah as StaticDrawUsage,
    ai as JSZip,
    aj as compressors,
} from './compressors.f6880dca.js';
const ParquetTypes = [
    'BOOLEAN',
    'INT32',
    'INT64',
...
const FieldRepetitionTypes = ['REQUIRED', 'OPTIONAL', 'REPEATED'];
...
function getSchemaPath(schema, name) {
    ...
    for (const part of name) {
        const child = tree.children.find(
            (child2) => child2.element.name === part
        );
        if (!child)
...
function readElement(reader, type) {
    switch (type) {
        ...
        case CompactType.LIST: {
            ...
            const boolType =
                elemType === CompactType.TRUE || elemType === CompactType.FALSE;
            const values = new Array(listSize);
            for (let i = 0; i < listSize; i++) {
                values[i] = boolType
                    ? readElement(reader, CompactType.BYTE) === 1
        ...
        case CompactType.STRUCT: {
            ...
            let lastFid = 0;
            while (true) {
                const [fieldType, fid, newLastFid] = readFieldBegin(
                    reader,
                    lastFid
                );
                lastFid = newLastFid;
...
function markGeoColumns(schema, key_value_metadata) {
    ...
    for (let i = 1; i < schema.length; i++) {
        ...
        const { logical_type, name, num_children, repetition_type, type } =
            element;
        if (num_children) {
            i += num_children;
            continue;
...
async function parquetMetadataAsync(
    ...
    if (metadataLength > asyncBuffer.byteLength - 8) {
        throw new Error(
            `parquet metadata length ${metadataLength} exceeds available buffer ${
                asyncBuffer.byteLength - 8
            }`
        );
    }
    ...
    const row_groups = metadata.field_4.map((rowGroup) => ({
        columns: rowGroup.field_1.map((column, columnIndex) => ({
            ...
            meta_data: column.field_3 && {
                ...
                encoding_stats: column.field_3.field_13?.map(
                    (encodingStat) => ({
                ...
                size_statistics: column.field_3.field_16 && {
                    unencoded_byte_array_data_bytes:
                        column.field_3.field_16.field_1,
...
function logicalType(logicalType2) {
    ...
    if (logicalType2?.field_18)
        return {
            ...
            algorithm:
                EdgeInterpolationAlgorithms[logicalType2.field_18.field_2],
        };
    return logicalType2;
}
...
function equals(a, b) {
    ...
    return Array.isArray(a) && Array.isArray(b)
        ...
        : typeof a === 'object' &&
              ...
              Object.keys(a).every((k) => equals(a[k], b[k]));
}
...
function canSkipRowGroup(filter, group, physicalColumns) {
    ...
    if ('$and' in filter && Array.isArray(filter.$and)) {
        return filter.$and.some((subFilter) =>
            canSkipRowGroup(subFilter, group, physicalColumns)
        );
    }
    ...
    for (const [field, condition] of Object.entries(filter)) {
        ...
        for (const [operator, target] of Object.entries(condition || {})) {
            ...
            if (
                operator === '$ne' &&
                equals(minVal, maxVal) &&
                equals(minVal, target)
            )
                return true;
            if (
                operator === '$in' &&
                Array.isArray(target) &&
                target.every((v) => v < minVal || v > maxVal)
            )
                return true;
            if (
                operator === '$nin' &&
...
function parquetPlan({
    ...
    for (const rowGroup of metadata.row_groups) {
        ...
        if (
            ...
            groupEnd > rowStart &&
            groupStart < rowEnd &&
            !canSkipRowGroup(filter, rowGroup, physicalColumns)
        ) {
            const ranges = [];
            ...
            const groupSize =
                ranges[ranges.length - 1]?.endByte - ranges[0]?.startByte;
            if (!columns && groupSize < columnChunkAggregation) {
                ...
                throw new Error(
                    `parquet columns not found: ${columns.join(', ')}`
                );
            }
        }
        groupStart = groupEnd;
...
function assembleLists(
    ...
    for (let i = 0; i < n; i++) {
        ...
        while (
            currentDepth &&
            (rep < currentRepLevel ||
                repetitionPath[currentDepth] !== 'REPEATED')
        ) {
            if (repetitionPath[currentDepth] !== 'REQUIRED') {
                containerStack.pop();
                currentDefLevel--;
        ...
        while (
            ...
            (currentDefLevel < def ||
                repetitionPath[currentDepth + 1] === 'REQUIRED')
        ) {
            currentDepth++;
            if (repetitionPath[currentDepth] !== 'REQUIRED') {
                const newList = [];
        ...
        for (let i = 0; i < miniblockPerBlock && outputIndex < count; i++) {
            ...
            if (bitWidth2) {
                ...
                while (miniblockCount && outputIndex < count) {
                    let bits =
                        (BigInt(reader.view.getUint8(reader.offset)) >>
                            bitpackPos) &
                        mask;
                ...
                if (miniblockCount) {
                    reader.offset += Math.ceil(
                        (miniblockCount * Number(bitWidth2) +
                            Number(bitpackPos)) /
                            8
                    );
                }
                ...
                for (
                    let j = 0;
                    j < valuesPerMiniblock && outputIndex < count;
                    j++
                ) {
                    value += minDelta;
                    output[outputIndex++] = int32 ? Number(value) : value;
        ...
        if ((c & 3) === 0) {
            ...
            if (len2 > 60) {
                if (pos + 3 >= inputLength) {
                    throw new Error(
                        'snappy error literal pos + 3 >= inputLength'
                    );
                }
...
function readDataPage(bytes, daph, { type, element, schemaPath }) {
    ...
    if (daph.encoding === 'PLAIN') {
        ...
        const bitWidth2 =
            type === 'BOOLEAN' ? 1 : view.getUint8(reader.offset++);
        if (bitWidth2) {
...
function readColumn(
    ...
    if (rowCount > selectEnd && lastChunk) {
        chunks[chunks.length - 1] = lastChunk.slice(
            0,
            selectEnd - (rowCount - lastChunk.length)
        );
    }
    return chunks;
}
function readPage(
    ...
    if (header.type === 'DATA_PAGE') {
        ...
        let values = convertWithDictionary(
            dataPage,
            dictionary,
            daph.encoding,
            columnDecoder
...
function readRowGroup(options, { metadata, columns }, groupPlan) {
    ...
    for (const { file_path, meta_data } of groupPlan.rowGroup.columns) {
        ...
        if (columnBytes > 1 << 30) {
            console.warn(
                `parquet skipping huge column "${meta_data.path_in_schema}" ${columnBytes} bytes`
            );
            continue;
        }
...
async function asyncGroupToRows(
    ...
    const columnDatas = await Promise.all(
        asyncColumns.map(({ data }) => data.then(flatten))
    );
...
function buildGeometry(bim) {
    ...
    console.log(
        'Created %d material groups',
        Array.from(materialGroups.keys()).length
    );
    const instancedMeshes = createInstances(
        bim,
        geometries,
        materials,
        transforms,
    ...
    const nonInstancedMeshes = createMergedAndSingleMeshes(
        bim,
        geometries,
        materials,
        transforms,
...
function createMergedAndSingleMeshes(
    bim,
    geometries,
    materials,
    transforms,
...
function computeMeshGeometries(bim) {
    ...
    for (let mi = 0; mi < meshCount; mi++) {
        ...
        const vEnd =
            mi + 1 < meshCount ? MeshVertexOffset[mi + 1] : vertexCount;
...
export { BimOpenSchemaLoader as B, loadBimGeometryFromZip as l };
