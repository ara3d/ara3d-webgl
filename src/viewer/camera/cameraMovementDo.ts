...
export class CameraMovementDo extends CameraMovement {
    ...
    private lockVector(position: THREE.Vector3, fallback: THREE.Vector3) {
        const x =
            this._camera.allowedMovement.x === 0 ? fallback.x : position.x;
        const y =
            this._camera.allowedMovement.y === 0 ? fallback.y : position.y;
        const z =
            this._camera.allowedMovement.z === 0 ? fallback.z : position.z;
