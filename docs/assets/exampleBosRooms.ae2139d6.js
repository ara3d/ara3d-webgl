import { B as BimOpenSchemaLoader, ew as MeshStandardMaterial, C as Color } from "./bimOpenSchemaLoader.1c0420b7.js";
...
function buildRoomTable(data) {
  ...
  for (const inst of data.Instances) {
    inst.visible = data.Resolver.GetInstanceCategoryName(inst) === "Rooms";
    if (inst.visible)
  ...
  for (const inst of roomInstances) {
    ...
    const params = data.Resolver.ParameterMap.get(inst.entity) ?? [];
    const names = ["__entity", "RoomName"];
    const values = [entityKey, data.Resolver.GetInstanceName(inst) ?? ""];
  ...
  const skippedSet = new Set(
    roomTable.getColumnNames().filter((n) => roomTable.isColumnMostlySame(n))
  );
  ...
  return { roomInstances, roomTable, entityToRowIndex, orderedColNames };
}
...
async function runExample() {
  ...
  let roomInstances = [];
  let roomTable;
  let entityToRowIndex = /* @__PURE__ */ new Map();
  let orderedColNames = [];
  ...
  function setModel(data) {
    ...
    currentRoom = null;
    materialCache.clear();
    const built = buildRoomTable(bimData);
    roomInstances = built.roomInstances;
    roomTable = built.roomTable;
    entityToRowIndex = built.entityToRowIndex;
    orderedColNames = built.orderedColNames;
    if (grid) {
      grid.setDataTable(roomTable.project({ columns: orderedColNames }));
      gradientChooser.setParameterNames(orderedColNames);
    }
    rebuild();
    refreshColorsFromCurrentChoice();
  }
  const initialData = await loader.load(
    "/ara3d-webgl/Snowdon Towers Sample Architectural.bos",
    { loadParameters: true }
  );
  bimData = initialData;
  const initialBuilt = buildRoomTable(bimData);
  roomInstances = initialBuilt.roomInstances;
  roomTable = initialBuilt.roomTable;
  entityToRowIndex = initialBuilt.entityToRowIndex;
  orderedColNames = initialBuilt.orderedColNames;
  grid = await showParameterGrid({
    host: gridHost,
  ...
  gradientChooser = showGradientChooser({
...
runExample();
//# sourceMappingURL=exampleBosRooms.ae2139d6.js.map
