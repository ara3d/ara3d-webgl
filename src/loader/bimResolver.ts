...
export class BimResolver {
  ...
  GetVal (rawVal: number, descType: number): any {
    ...
    if (descType == 2) {
      if (rawVal >= 0) { return this.GetEntityName(rawVal as EntityIndex) }
      return ''
    }
    return rawVal
  }
  ...
  ProcessParameters (table: BimParameterValues) {
    ...
    for (let i = 0; i < table.Value.length; i++) {
      ...
      const param = { Name, Value } as Parameter
      const tmp = this.ParameterMap.get(entityIndex)
      if (tmp === undefined) {
  ...
  * DescriptorIndices (): Iterable<DescriptorIndex> { for (let i = 0; i < this.DescriptorCount; i++) yield i as DescriptorIndex }
  ...
  first<T> (iterable: Iterable<T>, predicate: (value: T) => boolean, _default: T): T {
    for (const value of iterable) { if (predicate(value)) return value }
    return _default
  }
